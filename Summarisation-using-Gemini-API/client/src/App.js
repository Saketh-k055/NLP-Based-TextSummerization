...
const App = () => {
    ...
    const handleSummarize = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await axios.post('http://localhost:5000/api/summaries', { text });
            setSummary(response.data.summary);
            console.log('Summary received:', response.data.summary);
            applyTypingEffect(response.data.summary);
        } catch (error) {
    ...
    const applyTypingEffect = (text) => {
        if (summaryRef.current) {
            summaryRef.current.textContent = ''; // Clear previous content
            summaryRef.current.classList.add('reveal');
            summaryRef.current.style.animation = 'typing 2s steps(40, end), blink 0.75s step-end infinite';
            // Simulate typing effect
            let i = 0;
            const interval = setInterval(() => {
                if (i < text.length) {
                    summaryRef.current.textContent += text.charAt(i);
                    i++;
                } else {
      ...
      <div className="upload-form">
        ...
        <div class="container noselect">
          <div class="canvas">
            ...
            <div id="card">
              <p id="prompt">BriefBot</p>
              <div class="title">Text Summarising chatbot</div>
            </div>
          </div>
        </div>
        <br />
